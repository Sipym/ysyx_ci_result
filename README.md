...
> 注意：从这次流片班车开始，CICD的代码分支命名方式修改为当前班车接收代码的起始时间，格式为`年份+月份`，比如这次班车的分支就是`202302`。同时从这次班车开始，报告返回准备使用完整的8位学号命名方式。但是考虑到兼容性，目前本期的CICD仍将统一返回`ysyx_学号后六位`格式的目录结构，但是也支持`ysyx_八位学号`的提交，CICD会自动完成转换。

这是存放一生一芯CICD测试后返回报告的仓库。ysyx_submit中有`submit`和`report`两个目录，`submit`目录用于存放前端设计同学提交的处理器核代码的gitee子仓库，`report`目录存放SoC和后端团队对前端代码进行功能仿真和综合后返回的报告。运行一次完整的vcs测试时间大约在40-50分钟之间，一次完整dc测试时间在1-2个小时左右。由于一次测试时间较长，所以我们在`report`自己学号目录下的`state`文件记录有当前核在提交队列中的位置，具体格式为：
```txt
state: under test
```
表示当前核正在测试中，或者为：
```txt
state: wait [nums] cores
```
...
目前CICD仅支持使用commit来触发相对应测试任务，具体来说：
1. 使用 **`dc & vcs`** 的commit信息更新处理器核子仓库后，CICD会执行vcs和dc测试，并默认使用100MHz的时钟约束来跑dc。
2. 使用 **`dc & vcs constr=400`** 的commit信息更新处理器核子仓库后，CICD会执行vcs和dc测试，并使用400MHz的时钟约束来跑dc。
2. 使用 **`dc constr=400`** 的commit信息更新处理器核子仓库后，CICD**只会进行dc测试**，并使用400MHz的时钟约束来跑dc。

> 注意：'constr=xxx'中的时钟值xxx在100~800范围之间

3. 使用 **`wave`** 的commit信息更新处理器核子仓库后，CICD会返回vcs的仿真波形。考虑到仿真波形的大小，目前暂且只支持flash hello测试程序的波形，波形返回时间大约30~40分钟。波形文件为FST格式，可以使用GTKWave打开。文件命名方式为：`ysyx_学号后六位_flash_hello_待测试commit的提交时间.fst`。波形文件存储在网盘服务器上，并只会临时存储24小时，过时自动删除。网盘服务器地址为：http://wave.maksyuki.com/dir/63f43c002f3c1
...
├─ ysyx_XXXXXX(学号)          #每位同学上传的代码通过子模块的形式链接到该目录
   ...
   └─ ysyx_学号后六位.v        #提交的处理器核代码
   └─ ...                     #提交的其他相关文件 
```
### report 目录结构
```sh
├─ ysyx_XXXXXX(学号)          #每位同学的报告存放在自己学号的顶层目录下
   ├─ 2023-2-xx...xx:xx:xx    #根据不同的代码拉取时间，支撑团队返回的报告存放在不同的目录下
   │  ├─ dc_report            #DC综合报告
   │  └─ vcs_report           #VCS仿真报告
   └─ state                   #当前提交测试状态
```
...
### 一、确认功能正确性
* 阅读VCS报告report/ysyx_学号后六位/2023-2-xx...xx:xx:xx/vcs_report，确认compile有无错误，确认VCS流程是否存在**fail**。如果VCS程序未通过，则首先需要修改代码以确保功能正确，确认控制信号寄存器是否已做初始化。

### 二、消除DC综合报告Warning/Error
* 阅读DC综合报告report/ysyx_学号后六位/2023-2-xx...xx:xx:xx/dc_report的**SYNTHESIS REPORT**部分，关注报告中的Warning和Error。清除所有的Warning和Error，根据Warning的提示相应地去修改代码，对于无法清除的Warning，需要填写ysyx_submit目录下的[syn_warning.md](./syn_warning.md)，发给各自的组长，由组长反馈给支撑团队。
> 注意：必须要修改代码清除掉的Warning/Error类型为LINT-3、LINT-38、LINT-59、LINT-60、LINT-X4和LINT-58

### 三、确认综合后面积是否在约束范围内
前端设计的同学需要确保设计综合后的**Total cell area**在约束范围内，整个SoC面积应不超过**2.0平方毫米**，其中共享SRAM的大小约为**0.4平方毫米**，所以同学们自己核的面积需要不超过**1.6平方毫米**。
> 注意：由于测试环境有110nm切换到了28nm，以上面积约束大小需要进一步讨论后确定。
* 阅读DC综合报告report/ysyx_学号后六位/2023-2-xx...xx:xx:xx/dc_report的**AREA REPORT**部分，确认Total cell area是否满足约束范围内。
...
* 阅读DC综合报告report/ysyx_学号后六位/2023-2-xx...xx:xx:xx/dc_report的**SYNTHESIS REPORT**部分，查看DC综合流程在100MHz频率下是否**PASS**。
...
* 阅读DC综合报告report/ysyx_学号后六位/2023-2-xx...xx:xx:xx/dc_report的TIMING REPORT部分，报告中列出3条最长路径供参考，关于时序报告的格式可自行参考相关资料，例如：Vivado的时序报告。
